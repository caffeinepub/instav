...
import { useGetTopCreators, CreatorRanking } from '../hooks/useQueries';
  ...
  return (
    <section className="px-4 py-3">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h2 className="text-foreground font-semibold text-sm">Trending Creators</h2>
      </div>
      ...
      <div className="flex gap-3 overflow-x-auto pb-1 scrollbar-hide">
        {isLoading ? (
          ...
          <p className="text-muted-foreground text-xs py-2">No trending creators yet</p>
        ) : (
          creators.map((creator: CreatorRanking) => {
            const rank = Number(creator.rank);
            const name = creator.profile?.name ?? creator.principal.toString().slice(0, 8);
