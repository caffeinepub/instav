      ...
      >
        ...
        <div
          ...
          style={{
            background: `linear-gradient(160deg, 
              oklch(0.18 0.04 ${(parseInt(story.userId.replace(/\D/g, '') || '0')) * 60 + 200}) 0%, 
        ...
        <div className="absolute top-0 left-0 right-0 z-10 p-4 pt-5">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-white font-semibold text-sm leading-tight">{story.displayName}</p>
              <div className="flex items-center gap-1 mt-0.5">
                <Clock className="w-3 h-3 text-white/60" />
                <p className="text-white/60 text-xs">{timeAgo(story.timestamp)}</p>
              </div>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 px-8">
          ...
          <div className="text-center">
            <h2 className="text-white font-display font-bold text-2xl">{story.displayName}</h2>
          ...
          <div className="mt-4 px-6 py-3 rounded-2xl bg-white/10 backdrop-blur-sm border border-white/10">
            <p className="text-white/80 text-sm text-center">✨ Story content coming soon</p>
          </div>
        </div>
