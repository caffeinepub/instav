...
export default function EditorCanvas({ state }: EditorCanvasProps) {
  ...
  useEffect(() => {
    ...
    // Background
    const currentClip = state.clips.find(
      (c) => c.startTime <= state.currentTime && c.startTime + c.duration > state.currentTime
    ) ?? state.clips[0];
    ...
    // Draw stickers
    state.stickerLayers.forEach((sticker) => {
      if (sticker.startTime <= state.currentTime && sticker.startTime + sticker.duration > state.currentTime) {
        ctx.save();
    ...
    state.textLayers.forEach((layer) => {
      if (layer.startTime <= state.currentTime && layer.startTime + layer.duration > state.currentTime) {
        ctx.save();
