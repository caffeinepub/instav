...
import type { EditorState, Clip } from '@/pages/EditorPage';
...
export default function EditorTimeline({ state, onUpdate }: EditorTimelineProps) {
  ...
  const removeClip = (id: string) => {
    ...
    const newDuration = newClips.reduce((acc, c) => Math.max(acc, c.startTime + c.duration), 0);
    onUpdate({ clips: newClips, totalDuration: newDuration });
  };

  const addBlankClip = () => {
    const startTime = state.clips.reduce((acc, c) => Math.max(acc, c.startTime + c.duration), 0);
    const newClip: Clip = {
      id: `clip-${Date.now()}`,
      type: 'blank',
      name: 'Blank',
    ...
    <div className="p-3 space-y-3">
      ...
      <div className="relative overflow-x-auto rounded-xl bg-muted/50 border border-border/50">
        ...
        <div
          ...
          style={{ width: Math.max(300, state.totalDuration * PX_PER_SEC + 40) }}
          onClick={handleScrub}
        >
          {Array.from({ length: Math.ceil(state.totalDuration) + 1 }).map((_, i) => (
            ...
            >
              ...
              <span className="text-[9px] text-muted-foreground mt-0.5">{formatTime(i)}</span>
            </div>
          ))}
          {/* Playhead */}
          <div
        ...
        <div
          ...
          onClick={handleScrub}
        >
          <span className="absolute left-1 top-1 text-[9px] text-muted-foreground font-medium z-10">VIDEO</span>
        ...
        {state.textLayers.length > 0 && (
          <div
            ...
            style={{ width: Math.max(300, state.totalDuration * PX_PER_SEC + 40) }}
          >
            <span className="absolute left-1 top-1 text-[9px] text-muted-foreground font-medium">TEXT</span>
            {state.textLayers.map((layer) => (
              ...
              >
                <span className="text-[9px] text-white truncate">{layer.text}</span>
              </div>
            ))}
          </div>
        )}
        ...
        {state.audioTracks.length > 0 && (
          <div
            ...
            style={{ width: Math.max(300, state.totalDuration * PX_PER_SEC + 40) }}
          >
            <span className="absolute left-1 top-1 text-[9px] text-muted-foreground font-medium">AUDIO</span>
            {state.audioTracks.map((track) => (
              ...
              >
                <span className="text-[9px] text-white truncate">{track.name}</span>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="space-y-2">
        ...
        {state.clips.length === 0 ? (
          <p className="text-xs text-muted-foreground text-center py-4">No clips yet. Add media to get started.</p>
        ) : (
          state.clips.map((clip) => (
            <div key={clip.id} className="flex items-center gap-3 bg-muted/50 rounded-xl p-2.5 border border-border/30">
              <div className={cn('w-8 h-8 rounded-lg bg-gradient-to-br flex-shrink-0', TRACK_COLORS[clip.type])} />
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-foreground truncate">{clip.name}</p>
                <p className="text-[10px] text-muted-foreground">{formatTime(clip.duration)} · {clip.type}</p>
              </div>
              <button
