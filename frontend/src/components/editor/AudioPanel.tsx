...
import { Volume2, VolumeX, Trash2, Plus, Mic } from 'lucide-react';
import { Slider } from '@/components/ui/slider';
import type { EditorState, AudioTrack } from '@/pages/EditorPage';
...
export default function AudioPanel({ state, onUpdate }: AudioPanelProps) {
  ...
  return (
    <div className="p-3 space-y-4">
      ...
      <input ref={fileInputRef} type="file" accept="audio/*" className="hidden" onChange={addAudioTrack} />
      ...
      {state.audioTracks.length === 0 ? (
        ...
        <div className="space-y-3">
          {state.audioTracks.map((track) => (
            <div key={track.id} className="bg-muted/50 rounded-xl p-3 border border-border/30 space-y-3">
              <div className="flex items-center gap-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-muted-foreground">{formatTime(track.duration)}</p>
                </div>
                <button
                  onClick={() => updateTrack(track.id, { muted: !track.muted })}
                  className={cn('transition-colors', track.muted ? 'text-rose-400' : 'text-muted-foreground hover:text-foreground')}
                >
              ...
              <div className="flex items-center gap-3">
                ...
                />
                <span className="text-[10px] text-muted-foreground w-8 text-right">{track.volume}%</span>
              </div>
              ...
              <div className="h-8 bg-muted rounded-lg overflow-hidden flex items-center px-2 gap-0.5">
                {Array.from({ length: 40 }).map((_, i) => (
                  <div
                    key={i}
                    className={cn('flex-1 rounded-full', track.muted ? 'bg-muted-foreground/20' : 'bg-rose-400/60')}
