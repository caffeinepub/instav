...
const TRANSITIONS = [
  ...
  { id: 'slide-left', label: 'Slide Left', icon: '⬅️', desc: 'Slide to the left' },
  { id: 'slide-right', label: 'Slide Right', icon: '➡️', desc: 'Slide to the right' },
  { id: 'zoom-in', label: 'Zoom In', icon: '🔍', desc: 'Zoom into next clip' },
  { id: 'zoom-out', label: 'Zoom Out', icon: '🔎', desc: 'Zoom out to next clip' },
  { id: 'wipe', label: 'Wipe', icon: '🧹', desc: 'Wipe across the screen' },
  { id: 'dissolve', label: 'Dissolve', icon: '✨', desc: 'Dissolve between clips' },
  { id: 'flash', label: 'Flash', icon: '⚡', desc: 'Flash white between clips' },
  { id: 'spin', label: 'Spin', icon: '🌀', desc: 'Spin transition' },
  { id: 'blur', label: 'Blur', icon: '💫', desc: 'Blur transition' },
];

export default function TransitionsPanel({ state, onUpdate }: TransitionsPanelProps) {
  ...
  return (
    <div className="p-3 space-y-4">
      <div>
        <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">Transition Library</h3>
      ...
      {state.clips.length > 1 && (
        <div>
          <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">Apply to Clips</h3>
          <div className="space-y-2">
            {state.clips.slice(0, -1).map((clip, i) => {
              const key = `${i}-${i + 1}`;
              const current = state.transitions[key];
              return (
                <div key={clip.id} className="bg-muted/50 rounded-xl p-3 border border-border/30">
