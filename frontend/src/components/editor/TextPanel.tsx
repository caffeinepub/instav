...
const ANIMATIONS = ['none', 'fade-in', 'slide-up', 'typewriter', 'bounce', 'scale-pop'];
const COLORS = ['#ffffff', '#000000', '#f59e0b', '#ef4444', '#3b82f6', '#10b981', '#8b5cf6', '#ec4899'];
    ...
    <div className="p-3 space-y-4">
          ...
          {state.textLayers.map((layer) => (
            ...
            >
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-foreground truncate">{layer.text}</p>
                <p className="text-[10px] text-muted-foreground">{layer.fontFamily} · {layer.fontSize}px</p>
              </div>
              <button
      ...
      {selectedLayer && (
        <div className="bg-muted/50 rounded-xl p-3 border border-border/30 space-y-3">
          <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Edit Text</h3>
          ...
          <div className="flex items-center gap-3">
            ...
            <Slider
              ...
              className="flex-1"
            />
            <span className="text-[10px] text-muted-foreground w-8 text-right">{selectedLayer.fontSize}px</span>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={cn('p-2 rounded-lg transition-colors', selectedLayer.bold ? 'bg-amber-400/20 text-amber-400' : 'bg-muted text-muted-foreground hover:text-foreground')}
            >
              <Bold className="w-4 h-4" />
            </button>
            <button
              onClick={() => updateLayer(selectedLayer.id, { italic: !selectedLayer.italic })}
              className={cn('p-2 rounded-lg transition-colors', selectedLayer.italic ? 'bg-amber-400/20 text-amber-400' : 'bg-muted text-muted-foreground hover:text-foreground')}
            >
            ...
            {(['left', 'center', 'right'] as const).map((align) => {
              ...
              return (
                <button
                  ...
                  className={cn('p-2 rounded-lg transition-colors', selectedLayer.align === align ? 'bg-amber-400/20 text-amber-400' : 'bg-muted text-muted-foreground hover:text-foreground')}
                >
              ...
              {COLORS.map((c) => (
                <button
                  ...
                  className={cn(
                    ...
                    selectedLayer.color === c ? 'border-white scale-110' : 'border-transparent'
                  )}
          ...
          {/* Animation */}
          <div>
            <p className="text-[10px] text-muted-foreground mb-1.5">Animation</p>
            <div className="flex flex-wrap gap-1.5">
              {ANIMATIONS.map((anim) => (
                <button
                  key={anim}
                  onClick={() => updateLayer(selectedLayer.id, { animation: anim })}
                  className={cn(
