...
const EFFECTS = [
  { id: 'confetti', label: 'Confetti', emoji: '🎊', desc: 'Animated confetti burst' },
  { id: 'light-leaks', label: 'Light Leaks', emoji: '🌟', desc: 'Cinematic light leak overlay' },
  { id: 'glitch', label: 'Glitch', emoji: '⚡', desc: 'Digital glitch distortion' },
  { id: 'vignette-pulse', label: 'Vignette Pulse', emoji: '🔮', desc: 'Pulsing vignette effect' },
  { id: 'particle-burst', label: 'Particle Burst', emoji: '✨', desc: 'Sparkling particle explosion' },
  { id: 'letterbox', label: 'Letterbox', emoji: '🎬', desc: 'Cinematic black bars' },
  { id: 'film-grain', label: 'Film Grain', emoji: '📽️', desc: 'Vintage film grain texture' },
  { id: 'chromatic', label: 'Chromatic', emoji: '🌈', desc: 'Chromatic aberration split' },
];
...
export default function EffectsPanel({ state, onUpdate }: EffectsPanelProps) {
  ...
  return (
    <div className="p-3 space-y-4">
      <div>
        <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">Motion Graphics</h3>
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0">
                  <p className={cn('text-xs font-semibold truncate', isActive ? 'text-amber-400' : 'text-foreground')}>
                    {effect.label}
                  </p>
                  <p className="text-[10px] text-muted-foreground truncate">{effect.desc}</p>
                </div>
              </button>
            );
          })}
      ...
      {state.effects.length > 0 && (
        <div className="bg-muted/50 rounded-xl p-3 border border-border/30">
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Active Effects</p>
