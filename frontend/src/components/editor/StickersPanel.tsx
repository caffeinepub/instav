...
export default function StickersPanel({ state, onUpdate }: StickersPanelProps) {
  ...
  const updateSticker = (id: string, updates: Partial<StickerLayer>) => {
    onUpdate({
      stickerLayers: state.stickerLayers.map((s) => (s.id === id ? { ...s, ...updates } : s)),
    });
  };
    ...
    <div className="p-3 space-y-4">
      ...
      {state.stickerLayers.length > 0 && (
        <div className="space-y-2">
          <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Active Stickers</h3>
          {state.stickerLayers.map((sticker) => (
            <button
              key={sticker.id}
              onClick={() => setSelected(sticker.id === selected ? null : sticker.id)}
              className={cn(
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-foreground">Size: {sticker.size}px</p>
                <p className="text-[10px] text-muted-foreground">Rotation: {Math.round(sticker.rotation)}°</p>
              </div>
              <button
      ...
      {selectedSticker && (
        <div className="bg-muted/50 rounded-xl p-3 border border-border/30 space-y-3">
          <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Edit Sticker</h3>

          <div className="flex items-center gap-3">
            <span className="text-[10px] text-muted-foreground w-10">Size</span>
            <Slider
              value={[selectedSticker.size]}
              onValueChange={([v]) => updateSticker(selectedSticker.id, { size: v })}
              min={16}
              max={120}
              step={4}
              className="flex-1"
            />
            <span className="text-[10px] text-muted-foreground w-10 text-right">{selectedSticker.size}px</span>
          </div>

          <div className="flex items-center gap-3">
            <span className="text-[10px] text-muted-foreground w-10">Rotate</span>
            <Slider
              value={[selectedSticker.rotation]}
              onValueChange={([v]) => updateSticker(selectedSticker.id, { rotation: v })}
              min={-180}
              max={180}
              step={5}
              className="flex-1"
            />
            <span className="text-[10px] text-muted-foreground w-10 text-right">{Math.round(selectedSticker.rotation)}°</span>
          </div>

          <button
            onClick={() => updateSticker(selectedSticker.id, { rotation: 0, size: 48 })}
