...
const SHAPE_COLORS = ['#ffffff', '#000000', '#f59e0b', '#ef4444', '#3b82f6', '#10b981', '#8b5cf6', '#ec4899'];
...
export default function DesignPanel({ state, onUpdate }: DesignPanelProps) {
  ...
  const applyGradient = (gradient: string) => {
    ...
    if (colorMatch) {
      onUpdate({
        clips: state.clips.map((c, i) => (i === 0 ? { ...c, color: colorMatch[0] } : c)),
      });
    }
  };
  ...
  return (
    <div className="p-3 space-y-4">
      {/* Aspect ratio */}
      <div>
        <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">Canvas Size</h3>
          ...
          {ASPECT_RATIOS.map((ratio) => (
            ...
            >
              <div className={cn(
                ...
                ratio === '9:16' ? 'w-5 h-9' : ratio === '1:1' ? 'w-7 h-7' : 'w-9 h-5'
              )} />
              <span className={cn('text-[10px] font-medium', state.aspectRatio === ratio ? 'text-amber-400' : 'text-muted-foreground')}>
                {ratio}
              </span>
            </button>
          ))}
              ...
              {SHAPE_COLORS.map((c) => (
                <button
                  ...
                  className={cn(
                    ...
                    bgColor === c && !selectedGradient ? 'border-white scale-110' : 'border-transparent'
                  )}
            ...
            {GRADIENT_PRESETS.map((g) => (
              <button
                ...
                className={cn(
                  ...
                  selectedGradient === g.value ? 'border-white scale-105' : 'border-transparent'
                )}
      ...
      <div>
        ...
        <p className="text-[10px] text-muted-foreground mt-2 text-center">Tap a shape to add it to the canvas</p>
      </div>

      {/* Image upload */}
      <div>
        <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">Add Image</h3>
