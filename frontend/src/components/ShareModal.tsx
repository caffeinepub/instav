...
import { Post, useGetFriendsList, useGetUserProfile, useSendMessage } from '../hooks/useQueries';
  ...
  return (
    <button
      ...
      className={`flex items-center gap-3 w-full px-3 py-2.5 rounded-xl transition-colors ${
        selected ? 'bg-gold-500/15 border border-gold-500/40' : 'hover:bg-surface-2 border border-transparent'
      }`}
    >
      <AvatarPlaceholder
...
export default function ShareModal({ post, open, onOpenChange }: ShareModalProps) {
