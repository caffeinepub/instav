...
type PanelType = 'export' | 'trim' | 'filters' | 'effects' | 'stickers' | 'design';
...
const PANELS: { id: PanelType; label: string; icon: React.ReactNode }[] = [
  ...
  { id: 'design', label: 'Design', icon: <LayoutTemplate className="w-4 h-4" /> },
];
...
export default function EditorPage() {
  ...
  const [editorState, setEditorState] = useState<EditorState>(createDefaultState);
  ...
  const scrollTabs = (dir: 'left' | 'right') => {
    if (tabsRef.current) {
      tabsRef.current.scrollBy({ left: dir === 'left' ? -80 : 80, behavior: 'smooth' });
    }
  };

  // Build CSS filter string for video preview
  const cssFilter = useMemo(() => buildCssFilter(editorState.filters), [editorState.filters]);
    ...
    <div className="flex flex-col h-[calc(100vh-7rem)] overflow-hidden bg-background">
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-border bg-card flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-foreground text-sm truncate max-w-[160px]">{projectName}</span>
        </div>
          ...
          <div className="relative w-full h-full flex items-center justify-center">
            ...
            {editorState.effects.includes('glitch') && (
              <div
                ...
                style={{ background: 'repeating-linear-gradient(0deg, transparent, transparent 2px, rgba(255,0,0,0.1) 2px, rgba(255,0,0,0.1) 4px)' }}
              />
            )}
          </div>
        ) : (
      ...
      <div className="flex flex-col border-t border-border bg-card flex-shrink-0" style={{ height: '50%' }}>
        {/* Tab bar */}
